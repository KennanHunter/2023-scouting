...
export const StackValidationChecker: FC<PropsWithChildren> = ({
    children,
}) => {
    let allChildrenValid = useMemo(() => (Children.toArray(children) as ReactElement[]).reduce((allValid, child) => 
        allValid && (typeof child.props.error === 'undefined')
    , true), Children.toArray(children) );
