...
export const TBAImport: FC = () => {
    ...
    const [fetchSucceeded, setFetchSucceeded] = useState<boolean>(teamDB.db.length > 0);
    ...
    const fetchMatches = () => {
        ...
        fetch(`https://www.thebluealliance.com/api/v3/event/${eventKey}/matches`, {
            ...
            teamDB.push(safelyParsedJson.data.map((match) => ({
                    matchLevel: "Qualifications",
                    matchNumber: match.match_number,

                    "Red 1": Number.parseInt(match.alliances.red.team_keys[0].slice(3)),
                    "Red 2": Number.parseInt(match.alliances.red.team_keys[1].slice(3)),
                    "Red 3": Number.parseInt(match.alliances.red.team_keys[2].slice(3)),

                    "Blue 1": Number.parseInt(match.alliances.blue.team_keys[0].slice(3)),
                    "Blue 2": Number.parseInt(match.alliances.blue.team_keys[1].slice(3)),
                    "Blue 3": Number.parseInt(match.alliances.blue.team_keys[2].slice(3)),
                })));
            ...
            setFetchError("Failed to Fetch Matches, Please Check your Event Key");
            setFetchSucceeded(false);
    ...
    return (
        ...
        >
            ...
            <Select
                ...
                onChange={(value) => teamDB.setTeamPosition(value as TeamPosition)}
                my={4}
            />

            <Button fullWidth my={4} sx={{ flexGrow: 1 }} loading={isLoadingMatches} onClick={() => {
                if (!isLoadingMatches) fetchMatches()
            }}>
                Fetch Data
            </Button>

            <Button fullWidth my={4} sx={{ flexGrow: 1 }} onClick={() => {
                teamDB.clear()
                setFetchError("");
                setFetchSucceeded(false);
            }}>
            ...
            <Badge size="xl" color={(!fetchSucceeded && fetchError) ? "red" : ((fetchSucceeded) ? "green" : "gray")}>
                {(!fetchSucceeded && fetchError) ? `Error: ${fetchError}` : ((fetchSucceeded) ? "Data Imported Successfully!" : "No Data Imported")}
            </Badge>
            ...
            >
                ...
                <tbody>
                    {teamDB.db.sort((a, b) => (a.matchNumber > b.matchNumber) ? 1 : -1).map((match, index) => (
                        <tr key={`match#${index}`}>
