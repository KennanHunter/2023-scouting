...
const bytePacker = (data: Uint8Array): Uint8Array => {
    let chunks: number[][] = new Array(Math.ceil(data.length / 7)).fill([]).map(() => []);
    ...
    chunks.forEach((chunk) => {
        repackedData.push(chunk.reduce((bitmap, value, index) => bitmap += (((value > 0x7F) ? 1 : 0) << index), 0), ...chunk.map((value) => (value > 0x7F) ? (value - 0x80) : value));
    });
...
const byteUnpacker = (data: Uint8Array): Uint8Array => {
    let chunks: number[][] = new Array(Math.ceil(data.length / 8)).fill([]).map(() => []);
    ...
    chunks.forEach((chunk) => {
        ...
        unpackedData.push(...chunk.map((value, index) => ((chunkHeader & (1 << index)) != 0) ? (value + 0x80) : value));
    });
...
export const JSONGzip: Exporter<Uint8Array> = {
    match: {
        stringify: (db: MatchState[]) => {
            return bytePacker(pako.deflate(new TextEncoder().encode(JSON.stringify(db))));
        },
        ...
        parse: (rawInput: Uint8Array) => {
            ...
            try {
                rawQRCodeInput = JSON.parse(
                    new TextDecoder().decode(pako.inflate(byteUnpacker(rawInput)))
                );
